...
if __name__ == '__main__':
    ...
    var = data_parser.get_data(start_date="2017-08-18")
    # logging.debug(var)
    ...
    chart1 = ChartElement(data=rsi, chart_type=ChartType.LINE, plot=ChartAxis.DIFFERENT_AXIS, color=ChartColor.RED,
                          label="RSI")
    chart2 = ChartElement(data=stoch, chart_type=ChartType.LINE, plot=ChartAxis.ON_AXIS, color=ChartColor.BLUE,
                          label="STOCH")
    chart3 = ChartElement(data=sma, chart_type=ChartType.LINE, plot=ChartAxis.ON_AXIS, color=ChartColor.PINK,
                          label="SMA")
    chart4 = ChartElement(data=ema, chart_type=ChartType.LINE, plot=ChartAxis.ON_AXIS, color=ChartColor.PURPLE,
                          label="EMA")
    chart5 = ChartElement(data=macd, chart_type=ChartType.LINE, plot=ChartAxis.ON_AXIS, color=ChartColor.YELLOW,
                          label="MACD")
    chart6 = ChartElement(data=bbands, chart_type=ChartType.LINE, plot=ChartAxis.ON_AXIS, color=ChartColor.GREEN,
                          label="BBANDS")
    chart7 = ChartElement(data=pivot, chart_type=ChartType.LINE, plot=2, color="Magenta",
                          label="Pivot")
    charts = [chart1, chart2, chart3, chart4, chart5, chart6, chart7]
    params, data_with_indicators = data_parser.data_builder(var, charts=charts)
    logging.info(params)
