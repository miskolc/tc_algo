...
class Bhavcopy(object):
    ...
    def __init__(self, obs_date: datetime = None, cm: bool = False):
        """
        Initialises Bhavcopy download instance and URL to be downloaded.
        :param obs_date: datetime.datetim
                Date for which data is to be downloaded. Today, by default.
        :param cm: bool
                If True, download bhavcopy for equities (CM).
        """
    ...
    def get(self, overwrite: bool = False):
        """
        Downloads the file from the NSE. If file is already downloaded then cached version of the file is used.
        :param overwrite: bool
                If True, Overwrite the current file if it exists
        :return: DataFrame
                Reads the CSV file and returns it
        """
        self.__check_dir()
...
if __name__ == '__main__':
    ...
    while start_date <= end_date:
        Bhavcopy(start_date).get()
        # Bhavcopy(start_date, cm=True).get()
